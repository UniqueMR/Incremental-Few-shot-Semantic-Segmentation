...
class Dataset(object):
    def __init__(self, data_dir, fold, input_size=[321, 321] , normalize_mean=[0, 0, 0],
                 normalize_std=[1, 1, 1],prob=0.7,shots=1):
        # -------------------load data list,[class,video_name]-------------------
        self.shots = shots
        self.data_dir = data_dir
        self.new_exist_class_list, self.class_available = self.get_new_exist_class_dict(fold=fold)
    ...
    def get_new_exist_class_dict(self, fold):
        ...
        for fold in fold_list:
            ...
            while True:
                ...
                cat_list.append(cat)
        print('training categories available: ', set(cat_list))
        return new_exist_class_list, set(cat_list)
    ...
    def __getitem__(self, index):
        ...
        support_name_list = []
        while len(support_name_list) < self.shots:  # random sample a support data
            support_name = support_img_list[random.randint(0, len(support_img_list) - 1)]
            if support_name != query_name and support_name not in support_name_list:
        ...
        for support_name in support_name_list:
            ...
            scale_transform_mask = torchvision.transforms.Resize([scaled_size, scaled_size], interpolation=transforms.InterpolationMode.NEAREST)
            scale_transform_rgb = torchvision.transforms.Resize([scaled_size, scaled_size], interpolation=transforms.InterpolationMode.BILINEAR)
        ...
        support_rgbs = torch.stack(support_rgb_list, dim=0)
        support_masks = torch.stack(support_mask_list, dim=0)
        ...
        return query_rgb, query_mask, support_rgbs, support_masks, history_mask,sample_class,index
